...
{
	if ((g->map)[y / 65][x / 65] == '1' ||
		(g->map)[y / 65][x / 65] == 'C' || (g->map)[y / 65][x / 65] == 'E')
		return (0);
	return (1);
}

static void	flipper_two(int *step)
{
	if (step[0] == 65 && step[1] == 0)
	{
		step[0] = 0;
		step[1] = -65;
	}
	else if (step[0] == -65 && step[1] == 0)
	{
		step[0] = 0;
		step[1] = 65;
	}
	else if (step[0] == 0 && step[1] == 65)
	{
		step[0] = 65;
		step[1] = 0;
	}
...
static void	enemymaker(t_vars *g, int *step)
{
	...
	mlx_put_image_to_window(g->mlx, g->win, g->im[BH2].l, g->b2_x, g->b2_y);
}
